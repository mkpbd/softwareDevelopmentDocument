...
//@version=5
indicator("SMC Master Pro v20 | Institutional Upgrade", shorttitle="SMC Pro v20", overlay=true, max_boxes_count=500, max_lines_count=500, max_labels_count=500, max_bars_back=5000)

// ═══════════════════════════════════════════════════════════════
// v20 — Critical Fixes (built on v19)
//   FIX-1  Exit/flip labels now routed through f_pushExit pool (cap 80)
//          Prevents silent label truncation on long backtests (>500 label ceiling)
//   FIX-2  peakEquity updated at TP1 and TP2 partial closes (was only at SL/TP3)
//          Drawdown circuit now measures from rolling high-water mark, not stale base
//   FIX-3  CISD: removed internalTrend filter; added 0.4 ATR displacement gate
//          Reversal CISD in opposing trend is the most valuable signal — was blocked
//   FIX-4  Judas Swing: one-shot per London session (judasBullFired / judasBearFired flag)
//          Was firing every bar after setup, inflating liqOkBull/Bear entire session
//   FIX-5  BPR: requires temporal proximity ≤25 bars between bull and bear FVG
//          Was comparing last bull FVG vs last bear FVG regardless of time — false BPR
//   FIX-6  News blocker default changed false → true (safe default for live trading)
// ═══════════════════════════════════════════════════════════════
// v19 — Institutional Upgrade (built on v18)
//   REPAINT-1  FVG box push now gated on barstate.isconfirmed
//              Eliminates intrabar box creation (5–15% backtest inflation source)
//   REPAINT-2  TurtleSoup gated on barstate.isconfirmed
//              Removes intrabar var-state write to lastBullLiqBar
//   REPAINT-3  Trendline break gated on barstate.isconfirmed
//              Prevents wick-through permanent flag on live bar
//   RISK-1     Dynamic equity tracking (realEquity updates after each trade close)
//              Replaces static accountEquity input in sizing calculations
//   RISK-2     Open unrealized P&L included in dailyOk guard
//              Closes the bypass where open losses don't count against daily cap
//   RISK-3     Max drawdown circuit breaker (% from peak equity)
//              Prop-firm compliance: stops trading after equity drawdown threshold
//   RISK-4     Max lot/unit cap added to position sizing
//              Prevents runaway size on tight SL / high equity
//   RISK-5     Slippage model added to trading cost (price units, user-configurable)
//   RISK-6     Gap-through SL: exit at open price when candle gaps beyond SL
//   TREND-1    HTF trend uses EMA200 instead of EMA 20/50 (more stable, institutional)
//   SMT-1      SMT (Smart Money Tool) divergence detection added
//              Detects divergence between primary instrument and correlated pair
//   ICT-1      IDM (Inducement) detection added
//   ICT-2      CISD (Change In State of Delivery) detection added
//   ICT-3      Judas Swing detection (false session-open move) added
//   SCORE-1    Sweep self-validation fixed: sweep trigger no longer awards +2 to own score
//   LABEL-1    Signal entry labels pooled via f_pushMisc (closes label leak)
// ═══════════════════════════════════════════════════════════════
...
showAllFibs = input.bool(false, "All levels", group=grpFibR)
...
minConfluence = input.int(8, "Min Confluence", minval=1, maxval=25, group=grpSig)
...
ambigMode = input.string("Tick-approx by candle open", "Resolution", options=["Tick-approx by candle open", "SL-first (pessimistic)", "TP-first (optimistic)"], group=grpAmbig, tooltip="When same bar touches both SL and TP, which counts first.")
...
includeAlertDetails = input.bool(true, "Include details", group=grpAlert)
...
grpNews = "📰 News Blocker"
useNewsBlocker = input.bool(true, "Enable news blocker", group=grpNews)  // v20 FIX-6: default true (was false — dangerous in live trading)
newsWindows = input.string("0830,1000,1400", "HHMM list (NY)", group=grpNews)
newsBufferMin = input.int(15, "Buffer min ±", minval=1, maxval=60, group=grpNews)

grpGuard = "🛡️ Risk Guards"
useDailyLossCap = input.bool(true, "Daily loss cap", group=grpGuard)
...
// Keeps exit labels out of _miscLabels (cap 50) and below the 500-label global ceiling.
...
// FIX RE10045: Pine `and` does not short-circuit array.get — guard with nested if.
...
smtBull = useSMT and barstate.isconfirmed and not na(lastSL) and not na(prevSL) and lastSL < prevSL and not na(smtLastLow) and not na(smtPrevLow) and smtLastLow >= smtPrevLow

// SMT Bear: primary makes higher high, correlated pair makes lower high (divergence = reversal down)
smtBear = useSMT and barstate.isconfirmed and not na(lastSH) and not na(prevSH) and lastSH > prevSH and not na(smtLastHigh) and not na(smtPrevHigh) and smtLastHigh <= smtPrevHigh
...
var bool[]  bullOBIsMB      = array.new<bool>()
...
if showFibRet and not na(fibSH) and not na(fibSL) and barstate.islast
    // Direction: if primary trend is up, upswing; else downswing
    isUpswing = internalTrend == 1 ? true : internalTrend == -1 ? false : (fibSLIdx < fibSHIdx)
    swHi = fibSH
    swLo = fibSL
    rng = swHi - swLo
...
// v15 P0-2: not-broken guard
bullSweep_raw = _sweepConfirmed and detectSweeps and swingSignificant and not na(lastSL) and not lastSLBroken and low < lastSL and close > lastSL and (close > open or close > close[1])
bearSweep_raw = _sweepConfirmed and detectSweeps and swingSignificant and not na(lastSH) and not lastSHBroken and high > lastSH and close < lastSH and (close < open or close < close[1])
...
bullLiqGrab = _sweepConfirmed and detectLiqGrabs and swingSignificant and not na(lastSL) and not lastSLBroken and low < lastSL and close > lastSL and close > open
bearLiqGrab = _sweepConfirmed and detectLiqGrabs and swingSignificant and not na(lastSH) and not lastSHBroken and high > lastSH and close < lastSH and close < open
...
// ICT-1: IDM (Inducement) — engineered minor swing swept before real reversal
// Bull IDM: in downtrend, price pokes above recent high (traps shorts) then closes back below
// Bear IDM: in uptrend, price pokes below recent low (traps longs) then closes back above
...
if idmBull
    _idmL = label.new(bar_index, high, " IDM ▲ ", color=color.new(C_GOLD, 25), textcolor=C_TEXT_BLACK, style=label.style_label_down, size=sz_tiny, yloc=yloc.abovebar)
    f_pushMisc(_idmL)
if idmBear
    _idmS = label.new(bar_index, low, " IDM ▼ ", color=color.new(C_GOLD, 25), textcolor=C_TEXT_BLACK, style=label.style_label_up, size=sz_tiny, yloc=yloc.belowbar)
    f_pushMisc(_idmS)

// ICT-2: CISD (Change In State of Delivery)
// Bull CISD: prior bar was meaningful bearish delivery, current bar closes above prior bar's high
// Bear CISD: prior bar was meaningful bullish delivery, current bar closes below prior bar's low
// v20 FIX-3: removed internalTrend filter — CISD against trend is the most valuable reversal signal.
//            Added min displacement (0.4 ATR body) so doji-like CISD are suppressed instead.
_priorBearDelivery = close[1] < open[1] and (open[1] - close[1]) >= atrVal * 0.3
_priorBullDelivery = close[1] > open[1] and (close[1] - open[1]) >= atrVal * 0.3
_cisdBullDisp = (close - open) >= atrVal * 0.4 and close > open
_cisdBearDisp = (open - close) >= atrVal * 0.4 and close < open
cisdBull = barstate.isconfirmed and _priorBearDelivery and close > high[1] and _cisdBullDisp
cisdBear = barstate.isconfirmed and _priorBullDelivery and close < low[1]  and _cisdBearDisp
...
// v16 C1 fix: removed duplicate intTrend score (intTrend == internalTrend → was double-counting trend +3)
// v17 C-BUG-3 fix: don't score zone when the hard gate already forces it (zoneOkBull requires
// close < _zoneMid). A guaranteed +2 is not confluence — it's the entry condition restated.
bullScore := bullScore + ((not (useHardGate and hgRequireZone)) and close < _zoneMid ? 2 : 0)
bullScore := bullScore + bullPOIWeight
...
// v19 SCORE-1: when sweep IS the entry trigger, don't award +2 for it (self-validation fix)
// Sweep-triggered entries get +1 for the event; non-sweep entries with SSL context get +2
_bullSweepScore = bullSweep and not bullEngulf and not (includePinBarSignals and bullPin) ? 1 : (hasSSL or bullSweep) ? 2 : 0
bullScore := bullScore + _bullSweepScore
// v17 C-BUG-3 fix: drop KZ point when hard gate already requires KZ (guaranteed, non-discriminating)
bullScore := bullScore + ((not (useHardGate and hgRequireKZ)) and killzoneBoostConfluence and inAnyKZ ? 1 : 0)
bullScore := bullScore + (volBoost ? 1 : 0)
// v18 FIX-8: OR-dedup AMD+TS. v19: add SMT, IDM, CISD, Judas
bullScore := bullScore + ((amdLondonSweepLo or turtleSoupBull) ? 1 : 0)
bullScore := bullScore + (smtBull ? smtWeight : 0)
bullScore := bullScore + ((idmBear or cisdBull or judasBullSignal) ? 1 : 0)

bearScore = 0
bearScore := bearScore + (mtfBearBias ? 3 : trendHTF1 == -1 ? 2 : 0)
bearScore := bearScore + (internalTrend == -1 ? 2 : 0)
// v16 C1 fix: removed duplicate intTrend score
// v17 C-BUG-3 fix: same — zone point dropped when gate mandates it (zoneOkBear requires close > _zoneMid)
bearScore := bearScore + ((not (useHardGate and hgRequireZone)) and close > _zoneMid ? 2 : 0)
bearScore := bearScore + bearPOIWeight
...
// v19 SCORE-1 bear side
_bearSweepScore = bearSweep and not bearEngulf and not (includePinBarSignals and bearPin) ? 1 : (hasBSL or bearSweep) ? 2 : 0
bearScore := bearScore + _bearSweepScore
// v17 C-BUG-3 fix: drop KZ point when hard gate already requires KZ
bearScore := bearScore + ((not (useHardGate and hgRequireKZ)) and killzoneBoostConfluence and inAnyKZ ? 1 : 0)
bearScore := bearScore + (volBoost ? 1 : 0)
// v18 FIX-8: OR-dedup AMD+TS. v19: add SMT, IDM, CISD, Judas
bearScore := bearScore + ((amdLondonSweepHi or turtleSoupBear) ? 1 : 0)
bearScore := bearScore + (smtBear ? smtWeight : 0)
bearScore := bearScore + ((idmBull or cisdBear or judasBearSignal) ? 1 : 0)

// v16 C1 fix: dropped the duplicate trend "+1" from max (was 3+2+1+2+2+...)
// v17 C-BUG-3: zone(+2) and KZ(+1) only count when NOT gate-mandated → mirror in denominator
// so the dashboard "/max" reflects the real attainable score.
_zoneMax = (not (useHardGate and hgRequireZone)) ? 2 : 0
_kzMax   = (not (useHardGate and hgRequireKZ))   ? 1 : 0
// v18 FIX-7: volBoost is permanently 0 on forex/OTC instruments (_isForexOrOTC check in volBoost).
// The old formula included the +1 vol point unconditionally, making the displayed /max
// unattainable on FX. Now deduct it so the dashboard /max reflects true attainable score.
_volMax = _isForexOrOTC ? 0 : 1
// v19: + SMT(smtWeight) + ICT_extras(1 for idm/cisd/judas)
// Structure: MTF(3)+intTrend(2)+zone(_zoneMax)+POI(2)+htfPOI(htfPOIWeight)+pattern(1)+liq(max2)+kz(_kzMax)+vol(_volMax)+AMD_or_TS(1)+SMT(smtWeight)+ICT(1)
_smtMax = useSMT ? smtWeight : 0
maxScore_v15 = 3 + 2 + _zoneMax + 2 + htfPOIWeight + 1 + 2 + _kzMax + _volMax + 1 + _smtMax + 1
...
_unrealizedR_guard = posDirection != 0 and not na(posEntry) and _guardRisk > 0 ? (posDirection == 1 ? close - posEntry : posEntry - close) / _guardRisk : 0.0
...
var float posEntryOpen      = na    // v15 P1-4: entry bar's open price for ambig resolution
...
f_calcUnitSize(_entry, _sl) =>
    ...
    _units = _riskPerUnit > 0 ? _riskDollar / _riskPerUnit : 0.0
...
f_calcRawUnits(_entry, _sl) =>
    _riskDollar = realEquity * (riskPctEquity / 100.0)
    _riskPerUnit = math.abs(_entry - _sl)
    _rawFull = _riskPerUnit > 0 ? _riskDollar / _riskPerUnit : 0.0
    if useLotCap
...
if (pendingOpenLong or pendingFlipToLong) and _useNextBarFill
    if dailyOk and consecOk and kzTradeCapOk and drawdownOk
        posDirection := 1
        posEntry := open
        posEntryOpen := open
        posSL := f_calcSL(true)
...
if doFlipToLong
    ...
    // v16 C2 fix: book the flipped-out SHORT's realized R into risk guards
    // (close-price approx; with next-bar fill actual exit is next open). Was bypassing daily cap + consec losses.
    if not na(posEntry)
...
if doFlipToShort
    ...
    // v16 C2 fix: book the flipped-out LONG's realized R into risk guards
    if not na(posEntry)
...
//   "TP-first": optimistic
f_slFirst_long(_sl, _tp) =>
    if ambigMode == "SL-first (pessimistic)"
        true
    else if ambigMode == "TP-first (optimistic)"
        false
    else
        // Tick approx: distance from open to SL vs open to TP
        _dSL = math.abs(open - _sl)
        _dTP = math.abs(open - _tp)
        _dSL < _dTP

f_slFirst_short(_sl, _tp) =>
    if ambigMode == "SL-first (pessimistic)"
        true
    else if ambigMode == "TP-first (optimistic)"
        false
...
if enablePositions and posDirection == 1
    ...
    slFirstWins_long = slTouched_long and tp1Touched_long and f_slFirst_long(posSL, posTP1)
    ...
    if useMultiTP and not posTP1Hit and high >= posTP1 and not slFirstWins_long
        ...
        if useSizing and posUnitsRaw > 0
            realEquity := math.max(0.0, realEquity + realizedAtTP1 * posUnitsRaw)
    ...
    slFirstWinsTP2_long = slTouched_long_tp2 and tp2Touched_long and f_slFirst_long(posSL, posTP2)
    if tp2Touched_long and not slFirstWinsTP2_long
        ...
        if useSizing and posUnitsRaw > 0
            realEquity := math.max(0.0, realEquity + realizedAtTP2 * posUnitsRaw)
            peakEquity := math.max(peakEquity, realEquity)  // v20 FIX-2: rolling high-water mark
    ...
    if low <= posSL or open <= posSL
        ...
        if useSizing and posUnitsRaw > 0
            realEquity := math.max(0.0, realEquity + totalRealized * posUnitsRaw)
...
if enablePositions and posDirection == -1
    ...
    slFirstWins_short = slTouched_short and tp1Touched_short and f_slFirst_short(posSL, posTP1)
    ...
    slFirstWinsTP2_short = slTouched_short_tp2 and tp2Touched_short and f_slFirst_short(posSL, posTP2)
...
var table mtfTable = table.new(f_tablePos(tablePosition), 3, 14, bgcolor=C_BG_TABLE, border_width=1, border_color=C_BORDER_TBL, frame_color=C_GOLD, frame_width=2)
...
f_drawDashboard() =>
    if showMTFTable and barstate.islast
        ...
        table.cell(mtfTable, 2, 0, " v19 " + modeText + " ", text_color=C_TEXT_WHITE, bgcolor=C_BG_HEADER, text_size=size.small)
...
alertcondition(idmBull,         "IDM Bull",             "Inducement: downtrend high swept, bearish close — continuation down")
alertcondition(idmBear,         "IDM Bear",             "Inducement: uptrend low swept, bullish close — continuation up")
