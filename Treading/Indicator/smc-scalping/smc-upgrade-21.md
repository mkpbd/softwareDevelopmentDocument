...
//@version=5
indicator("SMC Master Pro v21 | Institutional Upgrade", shorttitle="SMC Pro v21", overlay=true, max_boxes_count=500, max_lines_count=500, max_labels_count=500, max_bars_back=5000)
...
// v21 FIX-10: margin utilization guard — warns when per-trade margin exceeds safe threshold
grpMargin = "⚖️ Margin Guard (v21)"
useMarginGuard  = input.bool(true,  "Show margin utilization", group=grpMargin, tooltip="Displays estimated margin % per trade in dashboard. Warning fires when > marginWarnPct.")
accountLeverage = input.float(100.0, "Account leverage (1:N)", minval=1.0, maxval=500.0, step=10.0, group=grpMargin, tooltip="Set your broker leverage so the dashboard can show margin % per open trade.")
marginWarnPct   = input.float(15.0, "Warn if margin > % equity", minval=1.0, maxval=100.0, step=1.0, group=grpMargin, tooltip="Dashboard row turns red when estimated margin exceeds this % of equity.")
...
if showFibRet and not na(fibSH) and not na(fibSL) and barstate.islast
    ...
    // v21 FIX-7: persist OTE zone bounds for confluence scoring on all bars
    _fibOteTop := math.max(fib_618, fib_786)
    _fibOteBot := math.min(fib_618, fib_786)
...
// v21 FIX-7: OTE confluence. +2 when trend-aligned in OTE; +1 counter-trend
bullScore := bullScore + (priceInOTE and internalTrend == 1 ? 2 : priceInOTE ? 1 : 0)
...
bullScore := bullScore + ((idmBear or cisdBull) ? 1 : 0)
// v21 FIX-6: Judas self-validation (mirrors v19 SCORE-1 for sweeps).
// When Judas IS the sole entry trigger, award 0 — cannot validate its own confluence.
...
bullScore := bullScore + (judasBullSignal and not _judasBullSoleTrigger ? 1 : 0)
...
// v21 FIX-7: OTE confluence bear side
bearScore := bearScore + (priceInOTE and internalTrend == -1 ? 2 : priceInOTE ? 1 : 0)
...
bearScore := bearScore + ((idmBull or cisdBear) ? 1 : 0)
// v21 FIX-6: Judas bear self-validation
...
bearScore := bearScore + (judasBearSignal and not _judasBearSoleTrigger ? 1 : 0)
...
// Structure: MTF(3)+intTrend(2)+zone(_zoneMax)+POI(2)+OTE(2)+htfPOI(htfPOIWeight)+pattern(1)+liq(max2)+kz(_kzMax)+vol(_volMax)+AMD_or_TS(1)+SMT(smtWeight)+ICT(1)+Judas(1)
_smtMax = useSMT ? smtWeight : 0
_oteMax = (showFibRet and showOTEZone) ? 2 : 0  // v21 FIX-7
maxScore_v15 = 3 + 2 + _zoneMax + 2 + _oteMax + htfPOIWeight + 1 + 2 + _kzMax + _volMax + 1 + _smtMax + 1 + 1
...
if (pendingOpenLong or pendingFlipToLong) and _useNextBarFill
    if dailyOk and consecOk and kzTradeCapOk and drawdownOk and not f_inNewsWindow()
        posDirection := 1
        posEntry := open
        posSL := f_calcSL(true)
        posOrigSL := posSL
...
if doFlipToLong
    ...
    if not na(posEntry)
        ...
        // v21 FIX-4: sync realEquity at flip exit so sizing/drawdown use correct equity
        if useSizing and posUnitsRaw > 0
            realEquity := math.max(0.0, realEquity + _flipClosePnL * posUnitsRaw)
...
if doFlipToShort
    ...
    if not na(posEntry)
        ...
        // v21 FIX-4: sync realEquity at flip exit
        if useSizing and posUnitsRaw > 0
            realEquity := math.max(0.0, realEquity + _flipClosePnL2 * posUnitsRaw)
            peakEquity := math.max(peakEquity, realEquity)
    f_clearPositionViz()
    if _useNextBarFill
        posDirection := 0
...
if posDirection != 0 and useSizing and posUnitsRaw > 0
    _unrealPnLUnits = posDirection == 1 ? (close - posEntry) * posSizeRemaining : (posEntry - close) * posSizeRemaining
    _unrealDollar = _unrealPnLUnits * posUnitsRaw
...
f_drawDashboard() =>
    if showMTFTable and barstate.islast
        ...
        table.cell(mtfTable, 2, 0, " v21 " + modeText + " ", text_color=C_TEXT_WHITE, bgcolor=C_BG_HEADER, text_size=size.small)
        ...
        // v21 FIX-10: margin utilization row
        if useMarginGuard and useSizing and posDirection != 0 and posUnitsRaw > 0 and posEntry > 0 and accountLeverage > 0 and realEquity > 0
            _marginUsed = (posUnitsRaw * posEntry) / (realEquity * accountLeverage) * 100.0
            _marginBg = _marginUsed > marginWarnPct ? C_LBL_BEAR : C_LBL_DARK
            _marginTxt = " ⚖️ MARGIN " + str.tostring(_marginUsed, "#.##") + "% " + (_marginUsed > marginWarnPct ? "⚠" : "✓")
            table.cell(mtfTable, 0, 14, " ⚖️ MARGIN ", text_color=C_TEXT_WHITE, bgcolor=_marginBg, text_size=size.normal)
            table.cell(mtfTable, 1, 14, " per trade ", text_color=C_TEXT_WHITE, bgcolor=_marginBg, text_size=size.normal)
            table.cell(mtfTable, 2, 14, _marginTxt, text_color=C_TEXT_WHITE, bgcolor=_marginBg, text_size=size.normal)
f_drawDashboard()
