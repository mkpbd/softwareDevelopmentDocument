...
//@version=5
indicator("SMC Master Pro v22 | Institutional Upgrade", shorttitle="SMC Pro v22", overlay=true, max_boxes_count=500, max_lines_count=500, max_labels_count=500, max_bars_back=5000)
...
// v22 HIGH-2 fix: htfPOI contributes to score only when useHTFPOI — denominator must match,
// else /max was unreachable with HTF POI disabled.
_htfPOIMax = useHTFPOI ? htfPOIWeight : 0
maxScore_v15 = 3 + 2 + _zoneMax + 2 + _oteMax + _htfPOIMax + 1 + 2 + _kzMax + _volMax + 1 + _smtMax + 1 + 1
...
// v22 MED-6: estimated margin % for a prospective entry (0 when gate disabled/no sizing)
f_marginPct(_px, _sl) =>
    _u = useSizing ? f_calcRawUnits(_px, _sl) : 0.0
    (useMarginBlock and useSizing and accountLeverage > 0 and realEquity > 0) ? (_u * _px) / (realEquity * accountLeverage) * 100.0 : 0.0
...
if _useNextBarFill and _flipExitDir != 0 and not na(_flipExitEntry)
    ...
    if _fxRisk > 0
        ...
        // v22 MED-4: dollar equity when sizing on, R-based compounding when off
        if useSizing and _flipExitUnitsRaw > 0
            realEquity := math.max(0.0, realEquity + _fxPnL * _flipExitUnitsRaw)
        else
            realEquity := math.max(0.0, realEquity * (1 + _fxR * riskPctEquity / 100.0))
...
if (pendingOpenLong or pendingFlipToLong) and _useNextBarFill
    _slPL = f_calcSL(true)
    if _okPendBase and (not useMarginBlock or not useSizing or f_marginPct(open, _slPL) <= marginMaxPct)
        posDirection := 1
        posEntry := open
        posSL := _slPL
        posOrigSL := posSL
...
if (pendingOpenShort or pendingFlipToShort) and _useNextBarFill
    ...
    if _okPendBase and (not useMarginBlock or not useSizing or f_marginPct(open, _slPS) <= marginMaxPct)
        posDirection := -1
        posEntry := open
        posSL := _slPS
        posOrigSL := posSL
    ...
    if _useNextBarFill
        ...
        if not na(posEntry)
            ...
            if _flipRisk > 0
                ...
                // v22 MED-4: dollar equity when sizing on, R-based when off
                if useSizing and posUnitsRaw > 0
                    realEquity := math.max(0.0, realEquity + _flipClosePnL * posUnitsRaw)
                else
                    realEquity := math.max(0.0, realEquity * (1 + _flipR * riskPctEquity / 100.0))
                peakEquity := math.max(peakEquity, realEquity)
        f_clearPositionViz()
...
if useMarginBlock and useSizing and f_marginPct(close, f_calcSL(true)) > marginMaxPct
    doOpenLong   := false
    doFlipToLong := false
        ...
        if not na(posEntry)
            ...
            if _flipRisk2 > 0
                ...
                if useSizing and posUnitsRaw > 0
                    ...
                    realEquity := math.max(0.0, realEquity * (1 + _flipR2 * riskPctEquity / 100.0))
                peakEquity := math.max(peakEquity, realEquity)
        f_clearPositionViz()
...
// v22 MED-6: margin gate for immediate (signal-close) fills
if useMarginBlock and useSizing and f_marginPct(close, f_calcSL(false)) > marginMaxPct
    doOpenShort   := false
    doFlipToShort := false
...
if enablePositions and posDirection == 1
    ...
    if low <= posSL or open <= posSL
        ...
        if useSizing and posUnitsRaw > 0
            ...
            peakEquity := math.max(peakEquity, realEquity)
        else if not useSizing and _riskPerUnit > 0
            realEquity := math.max(0.0, realEquity * (1 + (totalRealized / _riskPerUnit) * riskPctEquity / 100.0))
            peakEquity := math.max(peakEquity, realEquity)
        f_clearPositionViz()
        posDirection := 0
    else if high >= posTP3
...
if enablePositions and posDirection == -1
    ...
    if high >= posSL or open >= posSL
        ...
        if useSizing and posUnitsRaw > 0
            realEquity := math.max(0.0, realEquity + totalRealized * posUnitsRaw)
            peakEquity := math.max(peakEquity, realEquity)
        else if not useSizing and _riskPerUnitS > 0
            realEquity := math.max(0.0, realEquity * (1 + (totalRealized / _riskPerUnitS) * riskPctEquity / 100.0))
            peakEquity := math.max(peakEquity, realEquity)
        f_clearPositionViz()
        posDirection := 0
    else if low <= posTP3
...
f_drawDashboard() =>
    if showMTFTable and barstate.islast
        ...
        table.cell(mtfTable, 2, 0, " v22 " + modeText + " ", text_color=C_TEXT_WHITE, bgcolor=C_BG_HEADER, text_size=size.small)
