...
//@version=5
indicator("SMC Master Pro v25 | Institutional Upgrade", shorttitle="SMC Pro v25", overlay=true, max_boxes_count=500, max_lines_count=500, max_labels_count=500, max_bars_back=5000)

// v25: PULLBACK ENTRY — fixes late breakout-chasing. MSS/sweep ARMS the bias; LONG/SHORT arrow fires
//      only on the RETRACE into a fresh OB/FVG/OTE zone + confirmation candle. Entry pivot default 3
//      (was 5) to cut arm-lag. Removed trendlines + session-H/L-persistence drawing (token-limit trim).

// v24: RR VALIDATION ENGINE — measured-RR reject gate (logical SL anchor beyond sweep wick → nearest
//      opposing swing target), max-SL-distance sanity, discount/premium zone gate, win-probability +
//      ★ trade-grade label on every LONG/SHORT. Sub-1:minRR setups are now rejected, not displayed.
//      Parity with smc_strategy.pine v24 engine.
...
entrySwingSel = input.string("3", "Entry Structure Pivot (MSS)", options=["3", "5", "8", "12"], group=grpStruct, tooltip="v25: pivot length used ONLY for entry MSS qualification. Lower = faster, less pivot-confirmation lag (the '3' option confirms in 3 bars vs 5). Higher = fewer, cleaner arms. Default lowered to 3 to cut the late-arm lag now that pullback mode times the actual entry.")
entrySwingLen = entrySwingSel == "12" ? 12 : entrySwingSel == "8" ? 8 : entrySwingSel == "5" ? 5 : 3
...
// v24 RR VALIDATION ENGINE — measured-RR reject + logical SL anchor + opposing-liq target + win-prob/grade
// Parity with smc_strategy.pine. Rejects any LONG/SHORT whose measured RR (logical SL → nearest opposing
// swing) is below Min RR, whose SL is too wide, or which sits on the wrong side of the PD mid.
grpRRE = "🎯 RR Engine (v24)"
useRRengine    = input.bool(true,  "Enable RR validation engine", group=grpRRE, tooltip="v24: measure RR from a logical SL anchor (beyond the sweep wick / structural swing) to the nearest opposing swing. Reject setups below Min RR, with SL too wide, or outside a discount/premium zone.")
minRR          = input.float(1.5,  "Min RR (reject below)", minval=1.0, maxval=5.0, step=0.1, group=grpRRE)
maxSLatr       = input.float(4.0,  "Max SL distance (× ATR)", minval=1.0, maxval=12.0, step=0.5, group=grpRRE)
slAnchorSweep  = input.bool(true,  "Anchor SL beyond sweep wick", group=grpRRE)
rrSweepBars    = input.int(30,     "Sweep freshness (bars)", minval=3, maxval=200, group=grpRRE)
requireZoneRR  = input.bool(true,  "Require discount/premium zone", group=grpRRE, tooltip="Long only in discount (price ≤ PD mid), short only in premium.")
showTradeGrade = input.bool(true,  "Show probability + grade label", group=grpRRE)
...
// AUDIT (token offset for H1 force-close): BPR drawing block REMOVED to reclaim compile tokens
// (file was at 80025/80000 once). BPR was VISUAL-ONLY — never fed bullScore/bearScore (no priceInBPR
// term existed), default-off (showBPR_user=false), so removal changes no signal/entry logic.
// To restore: re-add a barstate.islast overlap-of-nearest-FVGs box; mind the 80k token ceiling.
...
// v24 RR engine: remember sweep wick extreme + bar for logical SL anchoring
...
// ═══════════════════════════════════════════════════════════════
// v25 PULLBACK ENTRY — fixes the "arrow prints ~5 bars late, chases the breakout, loses"
//   problem. Instead of firing on the breakout/MSS candle (which is pivot-lagged AND already
//   extended), a bullish MSS or sweep ARMS the bias; the LONG arrow then fires only when price
//   RETRACES into a fresh OB / FVG / OTE zone and a confirmation candle closes. Better fill,
//   tighter SL (RR engine anchors below the swept wick), no chase. Symmetric for shorts.
// ═══════════════════════════════════════════════════════════════
grpPB = "🎯 Pullback Entry (v25)"
usePullbackEntry = input.bool(true, "Pullback entry mode (enter at POI, not the breakout candle)", group=grpPB, tooltip="v25: after a bullish/bearish MSS or sweep arms the bias, the arrow fires only when price retraces into a fresh OB / FVG / OTE zone and a confirmation candle closes. Removes the late breakout-chasing entries. Turn OFF to restore the v24 breakout-trigger behaviour.")
pbArmBars        = input.int(20, "Setup stays armed N bars after MSS/sweep", minval=3, maxval=100, group=grpPB)
pbRequireConfirm = input.bool(true, "Require confirmation candle in zone", group=grpPB, tooltip="Long needs a bullish close (close>open and close>close[1]); short needs a bearish close. OFF = enter on first tap of the zone.")
pbUseOB  = input.bool(true, "Zone: Order Blocks", group=grpPB)
pbUseFVG = input.bool(true, "Zone: FVG", group=grpPB)
pbUseOTE = input.bool(true, "Zone: OTE", group=grpPB)

var int _pbBullArmBar = na
var int _pbBearArmBar = na
if entryMssBull or bullSweep
    _pbBullArmBar := bar_index
if entryMssBear or bearSweep
    _pbBearArmBar := bar_index
_pbBullArmed = not na(_pbBullArmBar) and (bar_index - _pbBullArmBar) <= pbArmBars
_pbBearArmed = not na(_pbBearArmBar) and (bar_index - _pbBearArmBar) <= pbArmBars

// retrace into a DISCOUNT bull POI / PREMIUM bear POI (OTE side-gated by the PD mid)
_pbBullOTE = priceInOTE and (na(_zoneMid) or close <= _zoneMid)
_pbBearOTE = priceInOTE and (na(_zoneMid) or close >= _zoneMid)
_pbInBullPOI = (pbUseOB and f_priceInBoxes(bullOBs, bullOBMitigated)) or (pbUseFVG and f_priceInBoxes(bullFVGs, bullFVGMitigated)) or (pbUseOTE and _pbBullOTE)
_pbInBearPOI = (pbUseOB and f_priceInBoxes(bearOBs, bearOBMitigated)) or (pbUseFVG and f_priceInBoxes(bearFVGs, bearFVGMitigated)) or (pbUseOTE and _pbBearOTE)

_pbBullConfirm = not pbRequireConfirm or (close > open and close > close[1])
_pbBearConfirm = not pbRequireConfirm or (close < open and close < close[1])

pbBull = usePullbackEntry and _pbBullArmed and _pbInBullPOI and _pbBullConfirm
pbBear = usePullbackEntry and _pbBearArmed and _pbInBearPOI and _pbBearConfirm

// v19: SMT divergence + CISD + Judas Swing added as entry triggers
// v25: when pullback mode is ON, the breakout-chasing triggers are replaced by the pullback trigger.
bullTrigger = usePullbackEntry ? pbBull : ((bullEngulf and dispOkBullEngulf) or (includePinBarSignals and bullPin and dispOkBullPin) or (includeSweepSignals and bullSweep) or (useSMT and smtBull) or cisdBull or judasBullSignal)
bearTrigger = usePullbackEntry ? pbBear : ((bearEngulf and dispOkBearEngulf) or (includePinBarSignals and bearPin and dispOkBearPin) or (includeSweepSignals and bearSweep) or (useSMT and smtBear) or cisdBear or judasBearSignal)
...
// ═══════════════════════════════════════════════════════════════
// v24 RR VALIDATION ENGINE — logical SL anchor + opposing-swing target + reject gate
//   (parity with smc_strategy.pine:402-436). Measures RR from a logical SL anchor — beyond the
//   fresh sweep wick when available, else the structural swing — to the nearest opposing swing
//   (first liquidity barrier). Rejects sub-minRR setups, over-wide stops, and wrong-zone entries.
// ═══════════════════════════════════════════════════════════════
f_winProb(_score, _rr, _mtf, _zone) =>
    _p = 38.0 + _score * 5.0 + (na(_rr) ? 0.0 : math.min(_rr, 5.0) * 4.0) + (_mtf ? 8.0 : 0.0) + (_zone ? 4.0 : 0.0)
    math.min(95.0, math.max(5.0, _p))
f_grade(_prob, _rr) =>
    _r = na(_rr) ? 0.0 : _rr
    (_prob >= 80 and _r >= 5) ? "★★★★★ Institutional" : (_prob >= 70 and _r >= 3) ? "★★★★ Premium" : (_prob >= 60 and _r >= 2) ? "★★★ Standard" : (_prob >= 50 and _r >= minRR) ? "★★ Marginal" : "★ Low"

_rrBuf = atrVal * slBufferAtrFrac
_bullSwpFresh = slAnchorSweep and not na(lastBullSweepBar) and (bar_index - lastBullSweepBar) <= rrSweepBars
_slAnchorLong  = _bullSwpFresh ? lastBullSweepLow - _rrBuf : (not na(lastSL) ? math.min(lastSL - _rrBuf, close - syminfo.mintick) : close - atrVal * atrMult)
_bearSwpFresh = slAnchorSweep and not na(lastBearSweepBar) and (bar_index - lastBearSweepBar) <= rrSweepBars
_slAnchorShort = _bearSwpFresh ? lastBearSweepHigh + _rrBuf : (not na(lastSH) ? math.max(lastSH + _rrBuf, close + syminfo.mintick) : close + atrVal * atrMult)

// logical target = nearest opposing swing beyond price (first barrier)
_rrTargetLong  = not na(lastSH) and lastSH > close ? lastSH : na
_rrTargetShort = not na(lastSL) and lastSL < close ? lastSL : na
_rrRiskLong  = close - _slAnchorLong
_rrRiskShort = _slAnchorShort - close
measuredRRLong  = (_rrRiskLong  > 0 and not na(_rrTargetLong))  ? (_rrTargetLong  - close) / _rrRiskLong  : na
measuredRRShort = (_rrRiskShort > 0 and not na(_rrTargetShort)) ? (close - _rrTargetShort) / _rrRiskShort : na

_rrZoneOkLong  = not na(_zoneMid) and close <= _zoneMid
_rrZoneOkShort = not na(_zoneMid) and close >= _zoneMid
slSaneLong  = not useRRengine or (_rrRiskLong  > 0 and _rrRiskLong  <= atrVal * maxSLatr)
slSaneShort = not useRRengine or (_rrRiskShort > 0 and _rrRiskShort <= atrVal * maxSLatr)
rrOkLong    = not useRRengine or (not na(measuredRRLong)  and measuredRRLong  >= minRR)
rrOkShort   = not useRRengine or (not na(measuredRRShort) and measuredRRShort >= minRR)
rrZoneOkLong  = not (useRRengine and requireZoneRR) or _rrZoneOkLong
rrZoneOkShort = not (useRRengine and requireZoneRR) or _rrZoneOkShort
probLong  = f_winProb(bullScore, measuredRRLong,  mtfBullBias, _rrZoneOkLong)
probShort = f_winProb(bearScore, measuredRRShort, mtfBearBias, _rrZoneOkShort)
gradeLong  = f_grade(probLong,  measuredRRLong)
gradeShort = f_grade(probShort, measuredRRShort)

// AUDIT M2: block entries when account-currency conversion is unresolved (valuePerPoint would
// silently assume 1.0 → mis-sizing). USD-quoted majors never set this flag → no regression.
// v24: RR engine gate (rrOk / slSane / rrZoneOk) folded into the final entry validity.
validBullEntry = validBullEntryRaw and cooldownOkBull and _barOk and hardGateBull and not newsBlocked and dailyOk and kzTradeCapOk and consecOk and drawdownOk and not _fxConvUnresolved and rrOkLong and slSaneLong and rrZoneOkLong
validBearEntry = validBearEntryRaw and cooldownOkBear and _barOk and hardGateBear and not newsBlocked and dailyOk and kzTradeCapOk and consecOk and drawdownOk and not _fxConvUnresolved and rrOkShort and slSaneShort and rrZoneOkShort
...
// v24 RR ENGINE: probability + RR + grade label (parity with smc_strategy.pine)
if showTradeGrade and useRRengine and validBullEntry
    f_pushMisc(label.new(bar_index, low, "▲ " + str.tostring(probLong, "#") + "%  RR " + (na(measuredRRLong) ? "—" : str.tostring(measuredRRLong, "#.#")) + "\n" + gradeLong, color=color.new(C_LBL_BULL, 20), textcolor=C_TEXT_WHITE, style=label.style_label_up, size=sz_small, yloc=yloc.belowbar))
if showTradeGrade and useRRengine and validBearEntry
    f_pushMisc(label.new(bar_index, high, "▼ " + str.tostring(probShort, "#") + "%  RR " + (na(measuredRRShort) ? "—" : str.tostring(measuredRRShort, "#.#")) + "\n" + gradeShort, color=color.new(C_LBL_BEAR, 20), textcolor=C_TEXT_WHITE, style=label.style_label_down, size=sz_small, yloc=yloc.abovebar))
...
f_calcSL(_isLong) =>
    // v24: when the RR engine is on, the executed SL uses the same logical anchor the RR gate
    // measured against — otherwise the displayed RR/grade would not match the real stop.
    if useRRengine
        _isLong ? _slAnchorLong : _slAnchorShort
...
// AUDIT H1: daily-loss & max-drawdown breakers must FLATTEN the open trade, not only block new
// entries. Without this a single open position can breach a prop daily/DD limit while the cap only
// stopped entries. Books the forced close at current price (with costs) exactly like the SL branch.
_ddNowCap = peakEquity > 0 ? (peakEquity - realEquity) / peakEquity * 100.0 : 0.0
_capHit = (useDailyLossCap and (dayRealizedR + _unrealizedR_guard) <= dailyLossR) or (useMaxDrawdown and _ddNowCap >= maxDrawdownPct)
if enablePositions and posDirection != 0 and _capHit and not na(posEntry)
    _capPnL  = (posDirection == 1 ? (close - posEntry) : (posEntry - close)) * posSizeRemaining - tradeCostPerUnit * posSizeRemaining
    _capTot  = posRealizedPnL + _capPnL
    _capRisk = math.abs(posEntry - posOrigSL)
    if _capRisk > 0
        _capR = _capTot / _capRisk
        dayRealizedR := dayRealizedR + _capR
        consecLosses := _capR < 0 ? consecLosses + 1 : 0
        realEquity := f_equityStep(realEquity, _capTot, posUnitsRaw, _capR, _capRisk)
        peakEquity := math.max(peakEquity, realEquity)
    f_pushExit(label.new(bar_index, close, " ⛔ CAP ", color=C_LBL_BEAR, textcolor=C_TEXT_WHITE, style=label.style_label_left, size=sz_small))
    f_clearPositionViz()
    posDirection := 0
