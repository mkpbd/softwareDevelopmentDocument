//@version=5
indicator("SMC Master Pro v15 | P0/P1 Audit Fixes", shorttitle="SMC Pro v15", overlay=true, max_boxes_count=500, max_lines_count=500, max_labels_count=500, max_bars_back=5000)

// ═══════════════════════════════════════════════════════════════
// v15 — P0/P1 Audit Fixes (built on v14)
//   P0-1  Primary trend driver = intSwingLen pivots (kills swingLen=8 repaint lag)
//         swingLen pivots reclassed as HTF-context only
//   P0-2  Sweep/grab require not-broken guard (no fire on already-broken swings)
//   P0-3  Position sizing: accountEquity + riskPct → output lot size on entry label
//   P1-4  Same-bar SL/TP ambiguity: tick-approx using entry-candle open side
//   P1-5  Displacement decoupled — engulf trigger only; pins exempt
//   P1-6  volBoost uses volume[1] on confirmed bar (no realtime repaint)
//   P1-7  Default fillMode = "Next bar open" — applies to INITIAL entries + flips
//   P2-8  Range filter mode: AND/OR configurable (default AND, stricter)
//   P2-9  POI age-decay weight (older POIs score less)
//   P2-10 Fib anchored at frozen PD reference (CHoCH leg) when available
//   P2-11 Magic constants exposed: MB recency, breaker pending age, iFVG age
//   P3-12 Single merged HTF security call (reduced from 18 tuples → 9)
//   P3-13 Generic f_priceInBoxes helper (replaces 4 mirror loops)
...
useNewsBlocker = input.bool(false, "Enable news blocker", group=grpNews)
...
// ═══════════════════════════════════════════════════════════════
f_getTrend(_tf) =>
    [_ema20Closed, _ema50Closed] = request.security(syminfo.tickerid, _tf, [ta.ema(close, 20)[1], ta.ema(close, 50)[1]], lookahead=barmerge.lookahead_off, gaps=barmerge.gaps_off)
...
// v15 P0-2: grabs also gated by not-broken
...
maxScore_v15 = 3 + 2 + 1 + 2 + 2 + htfPOIWeight + 1 + 2 + 1 + 1 + 1 + 1
...
bullTrigger = (bullEngulf and dispOkBullEngulf) or (includePinBarSignals and bullPin and dispOkBullPin) or (includeSweepSignals and bullSweep)
bearTrigger = (bearEngulf and dispOkBearEngulf) or (includePinBarSignals and bearPin and dispOkBearPin) or (includeSweepSignals and bearSweep)
...
validBullEntry = validBullEntryRaw and cooldownOkBull and _barOk and hardGateBull and not newsBlocked and dailyOk and kzTradeCapOk and consecOk
validBearEntry = validBearEntryRaw and cooldownOkBear and _barOk and hardGateBear and not newsBlocked and dailyOk and kzTradeCapOk and consecOk
...
var float posUnitSize       = 0.0   // v15 P0-3: position units/lots
...
if showMTFTable and barstate.islast
    ...
    table.cell(mtfTable, 2, 0, " v15 " + modeText + " ", text_color=C_TEXT_WHITE, bgcolor=C_BG_HEADER, text_size=size.small)
    ...
    // v15 Risk row: daily R + consec losses + units
    guardTxt = " R: " + str.tostring(dayRealizedR, "#.##") + " | L: " + str.tostring(consecLosses) + " "
    guardBg = (not dailyOk) or (not consecOk) ? C_LBL_BEAR : C_LBL_DARK
    table.cell(mtfTable, 0, 13, " 🛡️ GUARDS ", text_color=C_TEXT_WHITE, bgcolor=guardBg, text_size=size.normal)
    table.cell(mtfTable, 1, 13, useSizing ? " Size " : " Risk ", text_color=C_TEXT_WHITE, bgcolor=guardBg, text_size=size.normal)
    table.cell(mtfTable, 2, 13, useSizing and posUnitSize > 0 ? " " + str.tostring(posUnitSize, "#.##") + " " : guardTxt, text_color=C_TEXT_WHITE, bgcolor=guardBg, text_size=size.normal)
